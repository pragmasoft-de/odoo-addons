...
class eq_partner_ebid(models.Model):
    ...
    def proc_search_ebid_for_address(self, partner, ebid_settings, return_result = False):
        ...
        if (not searchRes):
            ...
            protocoll_for_partner = eq_prot_obj.sudo().search([('eq_res_id','=',partner.id), ('eq_model', '=', 'res.partner')])
            if (not protocoll_for_partner):
                prot_id = eq_prot_obj.eq_create_protocoll_entry(searchRes)
                ...
                prot_id = protocoll_for_partner.ids[0]
                protocol_record = eq_prot_obj.sudo().browse(prot_id)
                
                if (protocol_record):
                    protocol_record[0].eq_update_protocoll_entry(searchRes)
                ...
                if (not searchRes.requestOK):
                    ...
                    if ((searchRes.searchHits) and (len(searchRes.searchHits) > 0)):
                        for search_res in searchRes.searchHits:
                            protocol_pos_for_partner = eq_prot_position_obj.sudo().search([('eq_ebid_protocoll_id','=',prot_id), ('ebid_no', '=', search_res.ebid_no)])
